...
// Structured snapshot of the confirmed report's content for a tag - lets the
// Reports list show a read-only summary on click without re-parsing the PDF
// or re-deriving live (possibly since-edited) wizard state. Per-tag now that
// reports are per-tag.
export async function GET(
  ...
  const [row] = await db
    .select({ reportSummary: enquiryTags.reportSummary })
    .from(enquiryTags)
    .where(eq(enquiryTags.id, id))
    .limit(1);

  if (!row || !row.reportSummary) {
    return error("No saved report summary for this tag", 404);
  }
