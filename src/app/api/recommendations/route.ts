...
export async function POST(req: Request) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    results.push({
      ...
      flow: rec.flow,
      head: rec.head,
      bearingHousing: rec.bearingHousing,
      suctionHousing: rec.suctionHousing,
      jointType: rec.jointType,
      sealingType: rec.sealingType,
      moc: rec.moc,
      suctionSize: rec.suctionSize,
      deliverySize: rec.deliverySize,
      motor: rec.motor,
      driveSystem: rec.driveSystem,
      score: rec.score,
  ...
  return json({ selectionId: String(selection.id), recommendations: results });
}
